...
import { createKeyboardTexture } from "./deviceTextures";
...
const PHONE_SCREEN = { w: 1.5, h: 3.18 };
const LAPTOP_SCREEN = { w: 4.3, h: 2.72 };

/** A long page screenshot shown through a screen-shaped window; `scroll` 0..1 moves from top to bottom. */
type Scroller = { material: THREE.MeshBasicMaterial; visible: number; setScroll: (t: number) => void };

function createScroller(src: string, screen: { w: number; h: number }): Scroller {
  const material = new THREE.MeshBasicMaterial({ color: "#050506", toneMapped: false });
  const s: Scroller = { material, visible: 1, setScroll: () => {} };
  new THREE.TextureLoader().load(src, (tex) => {
    const img = tex.image as HTMLImageElement;
    tex.colorSpace = THREE.SRGBColorSpace;
    tex.anisotropy = 8;
    // Fraction of the page height that fits the screen at full width.
    s.visible = Math.min(1, img.width / (screen.w / screen.h) / img.height);
    tex.repeat.set(1, s.visible);
    tex.offset.set(0, 1 - s.visible);
    s.setScroll = (t) => {
      tex.offset.y = (1 - s.visible) * (1 - t);
    };
    material.map = tex;
...
export function Devices() {
  ...
  const a = useMemo(() => {
    ...
    return {
      phoneScreen: createScroller("/works/textures/phone-yadro.webp", PHONE_SCREEN),
      siteScreen: createScroller("/works/textures/laptop-torq.webp", LAPTOP_SCREEN),
      keyboardTex: createKeyboardTexture(),
      phoneBody: roundedSlab(1.62, 3.3, 0.27, 0.12, 0.035),
      phoneGlass: roundedPlane(PHONE_SCREEN.w, PHONE_SCREEN.h, 0.21),
      island: roundedPlane(0.42, 0.12, 0.06),
      bump: roundedSlab(0.7, 0.7, 0.18, 0.04, 0.015),
      base,
      lidBody: roundedSlab(4.6, 3.0, 0.16, 0.05, 0.02),
      lidScreen: roundedPlane(LAPTOP_SCREEN.w, LAPTOP_SCREEN.h, 0.06),
      keyboard,
      trackpad,
  ...
  useEffect(
    () => () => {
      ...
      [a.phoneScreen, a.siteScreen].forEach((s) => {
  ...
  useFrame((state) => {
    ...
    if (ph) {
      ...
      if (an) {
        ...
        a.phoneScreen.setScroll(easeInOut(clamp01((an.p - 0.15) / 0.7)));
      }
    }
    ...
    if (lp && lid.current) {
      ...
      if (an) {
        ...
        a.siteScreen.setScroll(easeInOut(clamp01((an.p - 0.38) / 0.42)));
      }
    }
  });
  ...
  return (
    <>
      <group ref={phone} visible={false}>
        ...
        <mesh geometry={a.phoneGlass} position={[0, 0, 0.096]} material={a.phoneScreen.material} />
      ...
      <group ref={laptop} visible={false}>
        ...
        <group ref={lid} position={[0, 0.08, -1.52]} rotation={[Math.PI / 2, 0, 0]}>
          ...
          <mesh geometry={a.lidScreen} position={[0, 1.52, 0.048]} material={a.siteScreen.material} />
        </group>
      </group>
    </>
  );
